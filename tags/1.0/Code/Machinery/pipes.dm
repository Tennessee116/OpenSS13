...
obj/machinery/pipes
	...
	// Return true as this is an unbroken pipe. All other machines and broken pipes return false.
	...
	buildnodes(var/obj/machinery/pipeline/line)
		...
		if(!line)			// if no pipeline specified, just needed to revalidate the local nodes
			return
	...
	examine()
		set src in view(1)
		if(usr && !usr.stat)
			usr << "[desc] The pipe is [damagetext()][(stat & MAINT)?" and the flanges are unfastened.":"."]"
	...
	attackby(obj/item/weapon/W, mob/user)
		...
		if (istype(W, /obj/item/weapon/wrench))
			if(stat & MAINT)
				stat &= ~MAINT
				user << "\blue You fasten the pipe flanges."
			else
				stat |= MAINT
				user << "\blue You unfasten the pipe flanges. The pipe can now be cut."
			...
			if(WT.welding)
				...
				if(stat & MAINT)
					if(WT.weldfuel > 3)
						...
						user << "\blue You need more welding fuel to cut the pipe."
					...
					if(health < MAXPIPEHEALTH)
						if(WT.weldfuel > 1)
							...
							user << "\blue Repairing the pipe."
							sleep(5)
							health = min(health+MAXPIPEHEALTH/10, MAXPIPEHEALTH)
							user << "\blue The pipe is now [damagetext()]."
							healthcheck()
							return
						else
							user << "\blue You need more welding fuel to repair the pipe."

					else
						user << "The pipe is not damaged."
						return
